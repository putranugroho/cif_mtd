...
class PengadaanNotifier extends ChangeNotifier {
        ...
        if (listTransaksi.isNotEmpty) {
          for (var i = 0; i < listGlAll.length; i++) {
            if (noDokTrans.text.isNotEmpty) {
              listTransaksiAdd.addAll(listTransaksi.where((e) => (e.dracc == listGlAll[i].nosbb) && e.status == "COMPLETED" && e.noDokumen == noDokTrans.text.trim() && e.tglValuta == DateFormat('y-MM-dd').format(tglValuta)).toList());
            } else {
              listTransaksiAdd.addAll(listTransaksi.where((e) => (e.dracc == listGlAll[i].nosbb) && e.status == "COMPLETED" && e.tglValuta == DateFormat('y-MM-dd').format(tglValuta)).toList());
            }
          }
        }
        isLoadingData = false;
  ...
  onChange() {
    if (pajak) {
      subtotal = int.parse(hargaBeli.text.replaceAll(",", "")) - int.parse(discount.text.replaceAll(",", "")) + int.parse(biaya.text.replaceAll(",", ""));
  ...
  gantipajak(bool value) {
    pajak = value;
    if (pajak) {
      ppn.text = (((int.parse(hargaBeli.text.replaceAll(",", "")) - int.parse(discount.text.replaceAll(",", "")) + int.parse(biaya.text.replaceAll(",", ""))) * double.parse(setupPajakModel!.ppn)) / 100).toInt().toString();
      pph.text = (((int.parse(hargaBeli.text.replaceAll(",", "")) - int.parse(discount.text.replaceAll(",", "")) + int.parse(biaya.text.replaceAll(",", ""))) * double.parse(setupPajakModel!.pph23)) / 100).toInt().toString();
      subtotal = int.parse(hargaBeli.text.replaceAll(",", "")) - int.parse(discount.text.replaceAll(",", "")) + int.parse(biaya.text.replaceAll(",", ""));
      total = (int.parse(hargaBeli.text.replaceAll(",", "")) - int.parse(discount.text.replaceAll(",", "")) + int.parse(biaya.text.replaceAll(",", "")) + (int.parse(ppn.text.replaceAll(",", "")))) - int.parse(pph.text.replaceAll(",", ""));
      notifyListeners();
    } else {
      ppn.text = "0";
      pph.text = "0";
