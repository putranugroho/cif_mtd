...
class LaporanHutangPiutangNotifier extends ChangeNotifier {
  ...
  Future getTransaksiPiutang() async {
    ...
    Setuprepository.setup(token, NetworkURL.viewnokontrak(), jsonEncode(data)).then((value) {
      Navigator.pop(context);
      if (value['status'].toString().toLowerCase().contains("success")) {
        for (Map<String, dynamic> i in value['data']) {
          listTransaksiPendingAdd.add(TransaksiPendModel.fromJson(i));
        }
        listTransaksiPendingAdd.sort((a, b) => a.createddate.compareTo(b.createddate));
        ...
        if (listTransaksi.isNotEmpty) {
          if (customerSupplierModel != null) {
            listTransaksiAdd = jenisTrans
                ? listTransaksi
                    .where(
                        (e) => ((double.parse(e.totalTagPokok) - double.parse(e.totalByrPokok))) == 0 && e.custsupp == customerSupplierModel!.noSif)
                    .toList()
                : listTransaksi
                    .where(
                        (e) => ((double.parse(e.totalTagPokok) - double.parse(e.totalByrPokok))) != 0 && e.custsupp == customerSupplierModel!.noSif)
            ...
            listTransaksiAdd = jenisTrans
                ? listTransaksi.where((e) => ((double.parse(e.totalTagPokok) - double.parse(e.totalByrPokok))) == 0).toList()
                : listTransaksi.where((e) => ((double.parse(e.totalTagPokok) - double.parse(e.totalByrPokok))) != 0).toList();
          }
        }
        isLoading = false;
  ...
  void changeTotal() {
    if (ppn) {
      double cekppn = (double.parse(nilaitransaksi.text.replaceAll("Rp ", "").replaceAll(".", "").replaceAll(",", ".")) *
          double.parse(setupPajakModel!.ppn) /
          100);
      nilaippn.text = "Rp ${FormatCurrency.oCcyDecimal.format(cekppn)}";
    }
    if (pph) {
      double cekpph = (double.parse(nilaitransaksi.text.replaceAll("Rp ", "").replaceAll(".", "").replaceAll(",", ".")) *
      ...
      try {
        ...
        if (response['status'].toString().toLowerCase().contains("success")) {
          ...
          return listCs.where((model) {
            final match = (model.nmSif.toLowerCase().contains(query.toLowerCase()));
  ...
  void onNilaiTransaksiChanged(int index, String newValue) {
    if (index == 0) {
      ...
      _debounce = Timer(const Duration(seconds: 2), () {
        ...
        if (ppn) {
          listNilaiPPN[index].text = FormatCurrency.oCcyDecimal.format((transaksiValue * double.parse(setupPajakModel!.ppn)) / 100);
        } else {
          listNilaiPPN[index].text = FormatCurrency.oCcyDecimal.format(0);
        }

        if (pph) {
          listNilaiPPH[index].text = FormatCurrency.oCcyDecimal.format((transaksiValue * double.parse(setupPajakModel!.pph23)) / 100);
        } else {
